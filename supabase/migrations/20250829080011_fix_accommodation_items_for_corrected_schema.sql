-- Create accommodation items for corrected accommodation schema
-- Date: 2025-08-29
...
DECLARE
  ...
  i INT;
BEGIN
  -- Get accommodation IDs for the corrected accommodations
  ...
  IF castle_view_id IS NOT NULL THEN
    ...
    DELETE FROM accommodation_items WHERE accommodation_id = castle_view_id;
  ...
  IF near_castle_id IS NOT NULL THEN
    -- Delete existing items first
    DELETE FROM accommodation_items WHERE accommodation_id = near_castle_id;
  ...
  IF ramparts_view_id IS NOT NULL THEN
    -- Delete existing items first  
    DELETE FROM accommodation_items WHERE accommodation_id = ramparts_view_id;
  ...
  IF garden_tipi_id IS NOT NULL THEN
    -- Delete existing items first
    DELETE FROM accommodation_items WHERE accommodation_id = garden_tipi_id;
